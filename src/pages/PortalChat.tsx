...
export default function PortalChat() {
  ...
  const { data: caseData } = useQuery({
    queryKey: ["portal-chat-case", session?.clientId],
    queryFn: async () => {
      const { data } = await pub.from("cases").select("id, case_type").eq("client_id", session!.clientId).limit(1).maybeSingle();
      return data;
    },
  ...
  useEffect(() => {
    if (messages.length === 0 && firstName) {
      setMessages([{
        role: "assistant",
        content: `Olá ${firstName}! Sou a LARA, assistente da Dra. Daiane. Posso ajudar com dúvidas sobre seu ${caseType}. O que gostaria de saber?`,
      }]);
    }
  }, [firstName, caseType]);
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: "assistant",
        content: data.reply || data.content || "Desculpe, não consegui processar. Tente novamente.",
      }]);
    } catch {
      setMessages(prev => [...prev, {
        role: "assistant",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <span style={{
            fontFamily: "var(--wizard-font-display)", fontSize: 17,
            fontWeight: 700, color: "var(--wizard-primary)", display: "block",
          }}>
            LARA
          </span>
          <span style={{
            fontFamily: "var(--wizard-font-body)", fontSize: 14,
            color: "#4A5568",
          }}>
            Assistente do Escritório Rosendo
          </span>
        </div>
      </div>
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              boxShadow: msg.role === "assistant" ? "0 1px 3px rgba(0,0,0,.04)" : "none",
        ...
        {loading && (
          <div style={{ display: "flex", justifyContent: "flex-start", marginBottom: 12 }}>
            <div style={{
              ...
              borderBottomLeftRadius: 4, border: "1px solid #E2E8F0",
            }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              background: input.trim() ? "#2B9E8F" : "#E5E7EB",
