...
const NAV_ITEMS = [
  ...
  { to: "/portal/lara", icon: MessageCircle, label: "Assistente" },
...
export default function ClientPortalLayout() {
  ...
  const { data: pendingCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await publicSupabase
        ...
        .in("case_id", (await publicSupabase
          .from("cases")
          .select("id")
          .eq("client_id", session!.clientId)
          .then(r => r.data?.map(c => c.id) || [])));
      return 0; // fallback, real count done below
    },
    enabled: false,
  });
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "var(--wizard-bg)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--wizard-font-body)" }}>
  ...
  if (error) {
    return (
      <div style={{ minHeight: "100vh", background: "var(--wizard-bg)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "var(--wizard-font-body)" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "32px 24px", maxWidth: 400, textAlign: "center", boxShadow: "0 2px 16px rgba(0,0,0,.06)" }}>
          <AlertCircle size={48} color="#C53030" style={{ margin: "0 auto 16px" }} />
          <p style={{ fontFamily: "var(--wizard-font-display)", fontSize: 20, fontWeight: 600, color: "var(--wizard-primary)", marginBottom: 8 }}>
            Acesso indisponível
          </p>
  ...
  return (
    <PortalSessionContext.Provider value={session}>
      <div style={{ minHeight: "100vh", background: "var(--wizard-bg)", fontFamily: "var(--wizard-font-body)", paddingBottom: 80 }}>
        <Outlet />
